...
func GetNodesOverwiew(c *gin.Context) {
	...
	for count, FluxNodeOverview := range FluxNodesOverview.FluxNode {
		...
		EstimatedTimeToWin := time.Now().Add(time.Duration(FluxNodeOverview.Rank) * time.Minute * 2)
		...
		es.Bulk("flux-node-overview", string(FluxNodeOverviewJson))
		nodeCount = count
	}
	stop := time.Now()
	log.Printf("######################################### STRART = %s , STOP = %s ################################################ Node = %d\n", start, stop, nodeCount)
